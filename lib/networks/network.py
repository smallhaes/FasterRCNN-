...
# 装饰器： 给当前层起个名字(如果没有的话)，找出当前层的输入，
# 喂给被装饰函数(op)，得到输出(可能多个输出？，每个输出都是{名字:值}的形式)，将这个输出作为后面层的输入
def layer(op):
...
class Network(object):
    ...
    # 输入： layer，具体来说是layer的名字，比如：'rpn_cls_score_reshape'等等
    # 输出： 返回某一层的输出
    def get_output(self, layer):
        try:
            # self.layers是个字典，key是输入或者输出的名字，value是对应的值
            layer = self.layers[layer]
        except KeyError:
            print self.layers.keys()
            raise KeyError('Unknown layer name fed: %s'%layer)
    ...
    def conv(self, input, k_h, k_w, c_o, s_h, s_w, name, relu=True, padding=DEFAULT_PADDING, group=1, trainable=True):
        # DEFAULT_PADDING：SAME
    	# group的值是将数据分块，从而在多GPU上运行
    	# 确保padding方式是'SAME'或者 'VALID'
    	# c_i：输入的通道数
    	# c_o：输出的通道数
        ...
        with tf.variable_scope(name) as scope:
            ...
            if group==1:
            	...
            	# 这个api有点老了
            	# 将input在channel维度上分成group份
            	# 分别卷积，然后再把卷积结果在channel的维度上concat起来
    ...
    def anchor_target_layer(self, input, _feat_stride, anchor_scales, name):
        ...
        with tf.variable_scope(name) as scope:
            # anchor_target_layer_py 返回值是： rpn_labels,rpn_bbox_targets,rpn_bbox_inside_weights,rpn_bbox_outside_weights
    ...
    def proposal_target_layer(self, input, classes, name):
        if isinstance(input[0], tuple):
            input[0] = input[0][0]
        with tf.variable_scope(name) as scope:
            # proposal_target_layer_py的返回值：采样得到的rois，rois对应的标签，rois相对其gt boxes的偏移量，偏移量对应的参数inside；偏移量对应的参数outside
    ...
    def reshape_layer(self, input, d,name):
    	# d是什么？？？ RPN中，对于fg和bg来说，d先是2再是18
        input_shape = tf.shape(input)
        if name == 'rpn_cls_prob_reshape':
             #[0]:batch_size    [1]:channel   [2]:height    [3]:width
        	 # reshape后是[0],d,[1]/d*[3],[2]
        	 # transpose后是[0],[1]/d*[3],[2],d
        	 ...
        	 # reshape后是[0],[d],[1]*([3]/d),[2]  跟上面有什么区别吗？？？
        	 # transpose后是[0],[1]*([3]/d),[2],[d]
             return tf.transpose(tf.reshape(tf.transpose(input,[0,3,1,2]),[input_shape[0],
    ...
    # 好像没用到这个函数，因为feature_extrapolating_op没有定义        
    @layer
    ...
    def fc(self, input, num_out, name, relu=True, trainable=True):
        ...
        with tf.variable_scope(name) as scope:
            ...
            if input_shape.ndims == 4:
                dim = 1
                # 第0维一般是batch size；将后面的维度乘起来
                for d in input_shape[1:].as_list():
                    dim *= d
                # 把通道数挪到第1维，再reshape，为什么这么做呢？？？
            ...
            # tf.nn.relu_layer是将全连接和relu激活写在一起了
    ...
    def softmax(self, input, name):
        ...
        if name == 'rpn_cls_prob':
        	# 如果是默认在最后一个维度上进行softmax，相当于是对input中每个点对应的通道数个值进行softmax
        	# 衡量了对于该点来说各个通道的重要性
