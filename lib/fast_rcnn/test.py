...
# 功能:将图片调整到适合输入network的形式: 去均值,缩放;     图片的blob形式:索引,高,宽,通道
# 输入: 图片
# 输出: 图片的blob格式;放缩倍数
...
# 根据proposal_alayer的输出rois,配合bbox_pred 进行了第二次回归
# 传进来的boxes也是None, 达到去掉gt的目的
def im_detect(sess, net, im, boxes=None):
    ...
    # __C.DEDUP_BOXES = 1./16               TEST.HAS_RPN: True
    ...
    if cfg.TEST.HAS_RPN:
        ...
        # 来自proposal_layer的proposals
        # 根据缩放比例,将bbox缩放至原图的大小(预处理图片的逆过程blob.prep_im_for_blob;      不是feature*16那个放缩)
...
# 很明显,这个thresh=0.05的大小是个非常关键的超参数啊
# 一张图片一张图片地处理
# max_per_image=300 每张图片最多检查300个bbox
def test_net(sess, net, imdb, weights_filename , max_per_image=300, thresh=0.05, vis=False):
    ...
    for i in xrange(num_images):
        ...
        for j in xrange(1, imdb.num_classes):
            ...
            # IoU >= this threshold)
            #__C.TEST.NMS = 0.3
            # nms后留下的符合条件的box对应的索引
            keep = nms(cls_dets, cfg.TEST.NMS)
            # 取出最终符合条件的box的信息:索引,分数
            cls_dets = cls_dets[keep, :]
            if vis:
                vis_detections(image, imdb.classes[j], cls_dets)
            # 类别j里索引为i的图片信息:该图片中的bbox坐标,该图片中的bbox分数
            all_boxes[j][i] = cls_dets
        if vis:
           plt.show()
        # Limit to max_per_image detections *over all classes*
        if max_per_image > 0:
            # 各个类别下索引为i的图片的boxes的得分
            image_scores = np.hstack([all_boxes[j][i][:, -1]
                                      for j in xrange(1, imdb.num_classes)])
            if len(image_scores) > max_per_image:
                # 如果bbox太多,则取出得分前300高的bbox
                image_thresh = np.sort(image_scores)[-max_per_image]
                for j in xrange(1, imdb.num_classes):
                    # 留下300个索引
                    keep = np.where(all_boxes[j][i][:, -1] >= image_thresh)[0]
                    # 类别j里索引为i的图片中最终留下的bbox:坐标,分数
